...
bool priority_comp_func (const struct list_elem *a,const struct list_elem *b,
	void *aux UNUSED);
...
{
	int new_priority = FIXED_TO_INT(SUB_FIXED_FIXED (SUB_FIXED_FIXED (PRI_MAX,
																																		MUL_FIXED_FIXED (
																																		thread->recent_cpu,
...
//    {
//      new_priority (list_entry(element, struct thread, elem), NULL);
...
thread_yield (void)
{
	...
	if (cur != idle_thread)
	{
		if (thread_mlfqs)
		{
			...
			list_insert_ordered (&ready_list_mlfqs, &cur->elem, &priority_comp_func,
													 NULL);
		} else
		{
...
thread_get_priority_helper (struct thread *t)
{
	...
	else
	{
		...
		{
			struct thread *thread_donated = list_entry (e,
			struct thread,
			donation_elem);
...
{
	return FIXED_TO_INT_ROUNDED(
				 MUL_FIXED_INT (thread_current ()->recent_cpu, 100));
}
